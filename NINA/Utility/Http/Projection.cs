...
namespace Projector
{
    class Projectify : BaseINPC
    {
        ...
        async public void CallToChildThread()
        {
            ...
            await AppVM.ImagingVM.SnapImage(appStatus);
        }
        ...
        async public void Projectionify(object sender, DoWorkEventArgs e)
        {
            ...
            listener.Start();
            Notification.ShowSuccess("HTTP server succesfully started.");
                    ...
                    if (param[0] == "0")
                    {
                        ...
                        if (rightAscension < 0)
                        {
                            if (Calculate(rightAscension, declination, TelescopeInfo.SiteLatitude, TelescopeInfo.SiteLongitude).Alt < 30)
                            {
                                ...
                                response.Close();
                                Notification.ShowError(param[3] + " is below 30 degrees.");
                                continue;
                            }

                            Notification.ShowSuccess("Slewing to " + param[3]);
                            ...
                            try { AppVM.ImagingVM.CancelSnapImage(new Object()); }
                            catch { }
