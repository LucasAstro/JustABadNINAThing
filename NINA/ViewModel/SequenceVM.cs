...
namespace NINA.ViewModel {
    ...
    internal class SequenceVM : DockableVM, ITelescopeConsumer, IFocuserConsumer, IFilterWheelConsumer, IRotatorConsumer, IFlatDeviceConsumer, IGuiderConsumer, ICameraConsumer, IWeatherDataConsumer {
        ...
        public async Task<PlateSolveResult> SlewToTarget(CaptureSequenceList csl, CancellationToken ct, IProgress<ApplicationStatus> progress)
        {
            ...
            if (csl.SlewToTarget)
            {
                ...
                plateSolveResult = await CenterTarget(csl, progress, _canceltoken);
            }
            return plateSolveResult;
        }

        public async Task<PlateSolveResult> CenterTarget(CaptureSequenceList csl, IProgress<ApplicationStatus> progress, CancellationTokenSource cal = default(CancellationTokenSource))
        {
            PlateSolveResult plateSolveResult = null;
            _canceltoken = cal;
            if (csl.CenterTarget)
